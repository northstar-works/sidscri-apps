...
REM ============================================================
REM make5_zip_SyncTool-WebServerToPackaged-Zip.bat
REM Put this BAT in: ...\sidscri-apps\
REM Creates ZIP in the SAME folder as this BAT:
REM   SyncTool-WebServerToPackaged-v<major.minor.patch> v<build>.zip
REM ZIP contents root:
REM   tools\SyncTool-WebServerToPackaged\...
REM Reads:
REM   .\tools\SyncTool-WebServerToPackaged\version.json
REM Excludes (recursively, guaranteed):
REM   .venv, __pycache__, *.pyc
REM Writes log:
REM   logs\SyncTool-WebServerToPackaged.log
REM ============================================================
...
set "PROJ=SyncTool-WebServerToPackaged"
set "TOOLS_DIR=%BASE%\tools"
set "SRC=%TOOLS_DIR%\%PROJ%"
...
if not exist "%SRC%\" (
  >>"%LOG%" echo ERROR: Tool folder not found
  goto :fail
)
...
mkdir "%STAGE%\tools\%PROJ%" >>"%LOG%" 2>&1

REM Copy tool to stage while excluding directories by NAME (recursive)
REM robocopy exit codes 0-7 = OK, >=8 = failure
robocopy "%SRC%" "%STAGE%\tools\%PROJ%" /E /R:1 /W:1 /NFL /NDL /NP /NJH /NJS /XD ".venv" "__pycache__" >>"%LOG%" 2>&1
set "RC=%ERRORLEVEL%"
...
del /s /q "%STAGE%\tools\%PROJ%\*.pyc" >>"%LOG%" 2>&1

REM ---------- ZIP FROM STAGE ----------
>>"%LOG%" echo Running 7z (from stage)...
pushd "%STAGE%" >nul

"%SEVENZIP%" a -tzip "%ZIPPATH%" "tools\%PROJ%" -r >>"%LOG%" 2>&1
set "ZERR=%ERRORLEVEL%"
